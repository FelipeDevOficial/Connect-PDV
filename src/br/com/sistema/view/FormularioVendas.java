    ...
    private void initComponents() {
        ...
        jPanel1Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                .addComponent(jLabel1, javax.swing.GroupLayout.PREFERRED_SIZE, 1711, javax.swing.GroupLayout.PREFERRED_SIZE)
...
try {
    ...
    Produtos obj = daot.BuscarProdutos(nome);

    if (obj.getDescricao() != null) {
        // Recuperar dados dos campos de texto
        int estoque = Integer.parseInt(txtEstoque.getText());
        int quantidade = Integer.parseInt(txtQtd.getText());
        double preco = Double.parseDouble(txtPreco.getText());
        double percentualDesconto = Double.parseDouble(txtDesconto.getText());
        String codigoProduto = txtCodigo.getText();

        // Calcular valores
        double desconto = preco * percentualDesconto / 100;
        double precoDesconto = preco - desconto;
        double subtotal = precoDesconto * quantidade;

        // Verificar se o código já está na tabelaCarrinho
        DefaultTableModel meus_produtos = (DefaultTableModel) tabelaCarrinho.getModel();
        boolean produtoJaAdicionado = false;

        for (int i = 0; i < meus_produtos.getRowCount(); i++) {
            if (meus_produtos.getValueAt(i, 0).toString().equals(codigoProduto)) {
                // Atualizar quantidade e subtotal para o produto já existente
                int qtdAtual = Integer.parseInt(meus_produtos.getValueAt(i, 2).toString());
                double subtotalAtual = Double.parseDouble(meus_produtos.getValueAt(i, 4).toString());
                
                int novaQtd = qtdAtual + quantidade;
                double novoSubtotal = subtotalAtual + subtotal;

                // Atualizar valores na tabela
                meus_produtos.setValueAt(novaQtd, i, 2); // Atualiza a quantidade
                meus_produtos.setValueAt(novoSubtotal, i, 4); // Atualiza o subtotal
        ...
        if (!produtoJaAdicionado) {
            // Produto não está na tabela, adiciona uma nova linha
    ...
    JOptionPane.showMessageDialog(null, "Erro ao processar valores numéricos. Verifique as entradas.");
    e.printStackTrace();
} catch (Exception e) {
